...
function Roles(props) {
  ...
  const newPerm = nn => {
    ...
    const common_permissions = props?.get_all_permission_data?.filter(elem =>
      nn?.find(({ permission_id }) => elem._id === permission_id)
    )
    //console.log(common_restaurants);

    const permission_data_edit = common_permissions
      ? common_permissions.map((item, key) => {
  ...
  useEffect(() => {
    ...
    props.get_all_permission_loading,
    props.add_role_loading,
    props.role_edit_loading,
    props.role_delete_loading,
    props.role_status_edit_loading,
  ])
            ...
            <Col className="col-12">
              <Card>
                <CardBody>
                  ...
                  {props.get_all_role_data ? (
                    props.get_all_role_data.length > 0 ? (
                      <DatatableTablesWorking
                        products={props.get_all_role_data}
...
const mapStateToProps = state => {
  const { get_all_permission_data, get_all_permission_loading } =
    state.Permissions
